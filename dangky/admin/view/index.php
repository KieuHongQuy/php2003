...
$('.timkiem button').click(function(event) {
			...
			window.location.href="index.php?controller=admin&action=index&keyword="+keyword;
        });
});      
</script>
<div id="page-wrapper">
    ...
    <?php if(count($data['data']['page']) > 0 && count($data['data']['product']) > 0){?>
            <nav aria-label="Page navigation example">
                <ul class="pagination t-center">
                    <li class="page-item">
                        <a class="page-link" href="index.php?controller=admin&action=index&page=<?=($_GET['page'] > 1) ? $_GET['page']-1 : '1'?>" aria-label="Previous">
                            <span aria-hidden="true">&laquo;</span>
                            <span class="sr-only">Previous</span>
                        </a>
                    </li>
                <?php for($i = 1;$i < $data['data']['page']+1;$i++){?>
                    <li class="page-item" ><a class="page-link <?=($_GET['page'] == $i)? 'acti': ''?>" href="index.php?controller=admin&action=index&page=<?=$i?>"><?php echo $i;?></a></li>
                 <?php }?>
                    <li class="page-item">
                        <a class="page-link" href="index.php?controller=admin&action=index&page=<?=($_GET['page'] >= 1 && $_GET['page'] + 1 <= $data['data']['page']) ? $_GET['page']+ 1 : '1'?>" aria-label="Previous">
